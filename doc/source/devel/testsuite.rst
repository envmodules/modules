...
``completion``
    ...
    currently covers bash and zsh, see `completion.00-init layout`_.
...
- ``005-init_ts.exp`` / ``006-procs.exp`` / ``010-environ.exp`` /
  ``011-save_test_env.exp`` set up paths (the ``bash`` and ``zsh`` binaries,
  the built :file:`init/bash_completion` and :file:`init/zsh-functions/_module`
...
- ``0NN-<shell>-procs.exp`` defines one ``completion_<shell>_start`` /
  ...
  label.
- ``0NN-<shell>.exp`` (e.g. ``021-bash.exp``, ``031-zsh.exp``) holds the
  actual test cases for that shell, calling only its own
  ``completion_<shell>_*`` procs plus the shared asserts.
...
00/030``), plus the ``999-cleanup.exp`` of every selected series. Passing a
bare series
